...
from .models import DiffFile, HunkInfo, ReviewComment, ReviewPriority
...
class CommentProcessor:
    ...
    def convert_to_review_comment(
        self, ai_response, diff_file: DiffFile, hunk: HunkInfo, hunk_index: int
    ) -> ReviewComment | None:
    ...
    def apply_comment_limits(self, comments: list[ReviewComment]) -> list[ReviewComment]:
        ...
        if dropped_due_to_file_cap:
            logger.info(
                f"Applied per-file cap: dropped {dropped_due_to_file_cap} comments exceeding {per_file_cap}/file"
            )
