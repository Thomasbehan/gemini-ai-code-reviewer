...
class GitHubClient:
    ...
    def _generate_review_summary(self, comments: List[ReviewComment]) -> str:
        ...
        if is_gordon:
            ...
            for priority in priority_order:
                ...
                if count > 0:
                    label = gordon_labels.get(priority, priority.title())
        ...
        # Executive summary: list the most important findings
        critical_and_high = [c for c in comments if c.priority.value in ("critical", "high")]
        if critical_and_high:
            if is_gordon:
                summary_parts.append("\n**The worst offenders:**")
            else:
                summary_parts.append("\n**Key findings:**")
            for comment in critical_and_high[:5]:  # Top 5 most important
                # Extract first sentence of the explanation
                body = comment.body or ""
                first_line = body.split("\n")[0].strip()
                if len(first_line) > 120:
                    first_line = first_line[:117] + "..."
                file_name = comment.path.split("/")[-1] if comment.path else "unknown"
    ...
    def _generate_approval_message(self) -> str:
        ...
        if is_gordon:
            return (
                "👨‍🍳🔥 **CHEF RAMSAY'S CODE KITCHEN**\n\n"
                "Well well well... I've gone through every line and you know what?\n\n"
                "**Finally! BEAUTIFUL code!** That's what I'm talking about! "
                "Clean, well-structured, properly handled — *chef's kiss* 👨‍🍳💋\n\n"
                "THIS is the standard I expect. Don't let it slip!\n\n"
                "> 👨‍🍳 Now get out of my kitchen before I change my mind!"
            )
        return (
    ...
    def get_existing_bot_comments(self, pr_details: PRDetails) -> List[Dict[str, Any]]:
        ...
        Only includes comments that:
        - Appear to be from the bot (have AI-SIG marker or match bot username)
        - Are NOT replies to other comments (in_reply_to_id is set)

        The in_reply_to_id filter is the key fix for duplicate follow-up spam: previously,
        the bot's own follow-up replies ("Previous issue: ... Status: Not Resolved") and
        resolution messages ("✅ This has been fixed") were being returned as "original"
        comments, causing the follow-up mechanism to re-verify them and post yet another
        round of replies — snowballing into 3-6 duplicate comments per original issue.
        """
        try:
            repo_obj = self._get_repo_with_retry(pr_details.repo_full_name)
            pr = self._get_pr_with_retry(repo_obj, pr_details.pull_number)
            ...
            logger.info(f"Found {len(bot_comments)} existing root bot review comments for PR #{pr_details.pull_number}")
            return bot_comments
