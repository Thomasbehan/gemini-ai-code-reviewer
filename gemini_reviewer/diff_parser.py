...
class DiffParser:
    ...
    def _parse_with_unidiff(self, diff_content: str) -> list[DiffFile]:
        ...
        try:
            ...
            if len(patch_set) == 0:
                logger.warning("PatchSet is empty! Diff preview (first 1000 chars):")
            ...
            for i, patched_file in enumerate(patch_set):
                logger.debug(
                    f"Processing patched file {i + 1}: {patched_file.source_file} -> {patched_file.target_file}"
                )
            ...
            logger.info(
                f"Unidiff parsing completed: {len(diff_files)} files processed, {self._skipped_files_count} skipped"
            )
            return diff_files
    ...
    def _convert_patched_file(self, patched_file: PatchedFile) -> DiffFile | None:
    ...
    def _convert_hunk(self, hunk: Hunk) -> HunkInfo | None:
    ...
    def _parse_hunk_header(self, header_line: str) -> HunkInfo | None:
    ...
    def filter_large_hunks(
        self, diff_files: list[DiffFile], max_lines_per_hunk: int = 500, max_hunks_per_file: int = 20
    ) -> list[DiffFile]:
        ...
        for diff_file in diff_files:
            ...
            for hunk in diff_file.hunks[:max_hunks_per_file]:  # Limit hunks per file
                if len(hunk.lines) > max_lines_per_hunk:
                    logger.debug(
                        f"Truncating large hunk in {diff_file.file_info.path} "
            ...
            if filtered_hunks:
                filtered_file = DiffFile(file_info=diff_file.file_info, hunks=filtered_hunks)
