...
class CodeReviewer:
    ...
    def __init__(self, config: Config):
        ...
        self.context_builder = ContextBuilder(
            self.github_client, self.diff_parser, project_context_budget=config.review.project_context_budget
        )
    ...
    async def review_pull_request(self, event_path: str) -> ReviewResult:
        ...
        try:
            ...
            try:
                ...
                logger.info(
                    f"Loaded {len(self._existing_comment_signatures)} existing AI comment signatures for duplicate avoidance during analysis"
                )
            ...
            if len(comments) == 0:
                if self.stats.files_processed > 0:
                    if self._is_followup_review:
                        ...
                        logger.info(
                            "No comments generated across all files - posting an approval review to acknowledge clean changes."
                        )
                        try:
    ...
    async def _get_pr_diff(self, pr_details: PRDetails) -> str:
        ...
        try:
            ...
            if getattr(self, "_force_fresh_review", False):
                ...
                diff_content = self.github_client.get_pr_diff(pr_details.owner, pr_details.repo, pr_details.pull_number)
    ...
    async def _parse_diff(self, diff_content: str) -> list[DiffFile]:
        ...
        try:
            ...
            logger.info(f"Parsed {stats['parsed_files']} files, skipped {stats['skipped_files']} files")
    ...
    async def _analyze_files_sequentially(
        self, diff_files: list[DiffFile], pr_details: PRDetails
    ) -> list[ReviewComment]:
            ...
            try:
                ...
                if file_comments:
                    if self._is_followup_review:
                        ...
                        logger.info(
                            f"Posting review for file: {diff_file.file_info.path} with {len(file_comments)} comments"
                        )
        ...
        with ThreadPoolExecutor(max_workers=max_workers) as executor:
            ...
            for future in as_completed(future_to_file):
                ...
                try:
                    ...
                    logger.debug(f"Completed analysis of {diff_file.file_info.path} ({len(file_comments)} comments)")
    ...
    def _analyze_single_file_sync(
        self, diff_file: DiffFile, pr_details: PRDetails, all_diff_files: list[DiffFile] | None = None
    ) -> list[ReviewComment]:
    ...
    async def _analyze_single_file(
        ...
        for hunk_index, hunk in enumerate(diff_file.hunks):
            try:
                ...
                ai_responses = self.gemini_client.analyze_code_hunk(hunk, context, prompt_template)

                self.stats.api_calls_made += 1

                # Convert AI responses to review comments
                for ai_response in ai_responses:
                    comment = self.comment_processor.convert_to_review_comment(ai_response, diff_file, hunk, hunk_index)
                    if comment:
                        ...
                        if sig and sig in getattr(self, "_existing_comment_signatures", set()):
                            ...
                            logger.debug(
                                f"Skipping generated duplicate comment on {comment.path} at pos {comment.position}"
                            )
                        else:
        ...
        if file_comments and getattr(self.config.review, "enable_verify_pass", True):
            try:
                ...
                if len(kept_indices) < len(file_comments):
                    ...
                    logger.info(f"Verify pass filtered {file_path}: {len(file_comments)} -> {len(verified)} finding(s)")
    ...
    async def _post_followup_replies(
        self, pr_details: PRDetails, diff_file: DiffFile, file_comments: list[ReviewComment]
    ) -> None:
        ...
        try:
            ...
            for c in prior_for_path:
                ...
                try:
                    c["_position"] = (
                        ...
                        else int(c.get("original_position"))
                        if c.get("original_position") is not None
                        else None
                    )
                except Exception:
                    c["_position"] = None
            ...
            def extract_referenced_issue(text: str) -> str:
                try:
                    ...
                    m = _re.search(
                        r"previous issue\s*:\s*(.+?)\s*(?:status\s*:\s*|$)", text, flags=_re.IGNORECASE | _re.DOTALL
                    )
                    if m:
            ...
            for cm in file_comments:
                ...
                if best is None:
                    try:
                        prior_for_path_sorted = sorted(
                            prior_for_path, key=lambda c: c.get("created_at") or "", reverse=True
                        )
    ...
    async def _resolve_completed_comments(self, pr_details: PRDetails) -> None:
        ...
        try:
            ...
            if considered > 0:
                logger.info(
                    f"Posted resolution replies on {resolved_count}/{considered} previous comment(s) for reviewed files."
                )
        except Exception as e:
            logger.debug(f"Resolution reply phase failed: {e}")
            return

    async def _create_github_review(
        self, pr_details: PRDetails, comments: list[ReviewComment], preferred_event: str | None = None
    ) -> bool:
        ...
        try:
            ...
            if preferred_event:
                ...
                if not limited_comments:
                    if total_comments > 0:
                        logger.info(
                            f"All {total_comments} comments were filtered by threshold/limits - posting comment about this"
                        )
                        event = "COMMENT"
                    else:
                    ...
                    logger.info(
                        f"Found {len(limited_comments)} comments (out of {total_comments} after dedupe) - requesting changes"
                    )
                    event = "REQUEST_CHANGES"

            # Pass both total and filtered comments so summary can be accurate
            success = self.github_client.create_review(
                pr_details, limited_comments, event, total_comments_generated=total_comments
            )
            if success:
                if limited_comments:
    ...
    def _build_change_summary(self, diff_files: list[DiffFile]) -> str:
        ...
        for diff_file in diff_files:
            ...
            summary_parts.append(f"- {file_path} [{change_type}] (+{additions}/-{deletions}){symbols_str}")
    ...
    def handle_review_comment_reply(self, event_path: str) -> bool:
        ...
        thread_text = "\n\n".join(
            f"{'REVIEWER' if m.get('is_bot') else m.get('author', 'human')}: {m.get('body', '')}" for m in messages
        )
